...
void	ft_spritecol(t_all *all, int k)
{
	...
	* all->text[4].width / all->sprwidth)/ 256);
	...
	while (++j < all->botline)
	{
		...
		if (all->sprytrans > 0 && all->sprytrans < all->bufferdist[k])
		{
			if (*(unsigned int *)(all->text[4].add + all->text[4].line_length
			* all->ytex + all->xtex * 4) != 1)
				*(unsigned int *)(all->addr + (j * all->line_length + k * 4))
				= *(unsigned int *)(all->text[4].add + 
...
void	ft_sprite2(t_all *all)
{
	...
	all->drawxstart = all->sprxscreen - all->sprwidth / 2;
	if (all->drawxstart < 0)
		all->drawxstart = 0;
